...
Generate word clouds of processed text
```{r}
#start with water plans

WaterPlanProc <- latest_data %>% filter(PlanType == "WaterPlanProcessed" & `Plan(1=YES) ` == 1) #27 plans

WaterPlanProc_subset <- WaterPlanProc %>%
  select(StateAbbreviation, Plan_Text) %>%
  rename(doc_id = StateAbbreviation,
         text = Plan_Text)

WaterPlanProc_Corpus <- Corpus(DataframeSource(WaterPlanProc_subset))

tdm <- TermDocumentMatrix(WaterPlanProc_Corpus)
m <- as.matrix(tdm)
...
wordcloud(d$word,d$freq, scale=c(8,.3),min.freq=100,max.words=Inf, random.order=F, rot.per=.15, colors=pal, vfont=c("sans serif","plain"))
